...
def age_gender_detector(frame):
    ...
    for bbox in bboxes:
        ...
        if int(index_ageList) > 1:
            print("warning time: ", warning_tim)
            current_time = datetime.now()
            try:
                time_that_should_pass = warning_tim + timedelta(seconds=10)
                if current_time > time_that_should_pass:
                    print("Dentro del ifffff" + str(warning_tim))
                    pb = Pushbullet(API_KEY)
                    push =pb.push_note("Advertencia", "Sujeto no identificado detectado en zona prohibida")
                    warning_tim = current_time
                else:
                    pass
            except Exception as e:
                print(e)
                warning_tim = current_time
                print("Dentro del except " + str(warning_tim))
            # pb = Pushbullet(API_KEY)
            # push =pb.push_note("Advertencia", "Sujeto no identificado detectado en zona prohibida")
        # #         push =pb.push_note("Advertencia", "Sujeto no identificado detectado en zona prohibida")
        # #     if warning_tim == None:
        # #         warning_tim = datetime.now()
        # #         #warning_tim = now.strftime("%H:%M:%S")
        # #         pb = Pushbullet(API_KEY)
        # #         push =pb.push_note("Advertencia", "Sujeto no identificado detectado en zona prohibida")
        # #     current_time = datetime.now()
        # #     #current_time = now.strftime("%H:%M:%S")
        # #     if current_time > (warning_tim + timedelta(seconds=30)):
        # #         pb = Pushbullet(API_KEY)
        # #         push =pb.push_note("Advertencia", "Sujeto no identificado detectado en zona prohibida")
        # #         warning_tim = None
    return frameFace
